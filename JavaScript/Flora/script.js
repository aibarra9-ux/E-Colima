...
function renderizarPublicaciones() {
    ...
    if (publicaciones.length === 0) {
        const mensajeNoResultados = idiomaActual === 'en' 
            ? "No plants or posts found matching your search."
            : "No se encontraron plantas o publicaciones que coincidan con la búsqueda.";
...
async function cargarPublicaciones() {
    ...
    try {
        ...
        if (idiomaActual === 'en' && typeof window.traducirPublicaciones === 'function') {
            console.log("[Flora] Traduciendo publicaciones con función global...");
...
window.addEventListener('idioma-cambiado', () => {
    console.log("[Flora] Idioma cambiado, recargando publicaciones...");
    cargarPublicaciones();
});
