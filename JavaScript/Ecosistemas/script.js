...
function renderizarPublicaciones() {
    ...
    if (publicaciones.length === 0) {
        const mensajeNoResultados = idiomaActual === 'en' 
            ? "No ecosystems or posts found matching your search."
            : "No se encontraron ecosistemas o publicaciones que coincidan con la búsqueda.";
...
async function cargarPublicaciones() {
    ...
    try {
        ...
        if (idiomaActual === 'en' && typeof window.traducirPublicaciones === 'function') {
            console.log("[Ecosistemas] Traduciendo publicaciones con función global...");
...
window.addEventListener('idioma-cambiado', () => {
    console.log("[Ecosistemas] Idioma cambiado, recargando publicaciones...");
    cargarPublicaciones();
});
