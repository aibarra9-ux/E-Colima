...
function renderizarPublicaciones() {
    ...
    if (publicaciones.length === 0) {
        const mensajeNoResultados = idiomaActual === 'en' 
            ...
            : "No se encontraron consejos que coincidan con la búsqueda.";
    ...
    publicaciones.forEach((post, i) => {
        ...
        if (post.tipo_media === 'video') {
            ...
            htmlMultimedia = `<img class="imagen-publicacion" src="${post.imagen}" alt="${tituloMostrar}">`;
        }
...
async function cargarPublicaciones() {
    ...
    try {
        ...
        if (idiomaActual === 'en' && typeof window.traducirPublicaciones === 'function') {
            console.log("[Consejos] Traduciendo publicaciones con función global...");
...
window.addEventListener('idioma-cambiado', () => {
    console.log("[Consejos] Idioma cambiado, recargando publicaciones...");
    cargarPublicaciones();
});
