...
const categoriaId = "2"; // ID único asignado a Fauna en tu BD
...
function renderizarPublicaciones() {
    ...
    if (publicaciones.length === 0) {
        const mensajeNoResultados = idiomaActual === 'en' 
            ? "No animals or posts found matching your search."
            : "No se encontraron animales o publicaciones que coincidan con la búsqueda.";
    ...
    publicaciones.forEach((post, i) => {
        ...
        if (i === 0) {
            ...
            // Reutilizamos el elemento multimedia adaptando las clases para el contenedor grande
...
async function cargarPublicaciones() {
    // 1. Construimos la ruta base apuntando de forma fija a la categoría general de Fauna
    ...
    try {
        ...
        if (idiomaActual === 'en' && typeof window.traducirPublicaciones === 'function') {
            console.log("[Fauna] Traduciendo publicaciones con función global...");
...
window.addEventListener('idioma-cambiado', () => {
    console.log("[Fauna] Idioma cambiado, recargando publicaciones...");
    cargarPublicaciones();
});
