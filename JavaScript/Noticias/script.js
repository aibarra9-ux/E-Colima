...
fetch('obtener_publicaciones.php?categoria_id=4')
    .then(response => response.json())
    .then(publicaciones => {
        if (publicaciones.length === 0) {
            contenedor.innerHTML = '<p style="text-align:center;color:white;padding:50px;">No hay publicaciones aún en Noticias.</p>';
            return;
        }
        ...
        publicaciones.forEach((post, i) => {
            ...
            setTimeout(() => { tarjeta.classList.add("mostrar"); }, 200 * i);
        });
    })
    .catch(error => {
