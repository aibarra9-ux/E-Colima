...
// Si no se pasa ni categoría ni subcategoría, usamos por defecto la que corresponda a Fauna (ej: 4 o la que tengas en tu BD)
if ($categoria_id === 0 && $subcategoria_id === 0) {
    $categoria_id = 2; // 💡 Nota: Cambié de 5 a 4 suponiendo que Fauna tiene otra ID, ajústalo si no es así.
}
...
while($row = $resultado->fetch_assoc()) {
    // Formateamos la ruta del recurso multimedia de antemano
    ...
    $publicaciones[] = [
        ...
        'imagen' => $rutaMedia,
        'tipo_media' => !empty($row['tipo_media']) ? $row['tipo_media'] : 'imagen', // 🌟 NUEVO: Enviamos el tipo de media al JS de Fauna
        'autor' => $row['autor'],
